...
export async function Event(message: Message, client: Qwik) {
    ...
    if (data && data.automod?.anti_invites === true) {
        ...
        if (/(https?:\/\/)?(www.)?(discord.(gg|io|me|li|link|plus)|discorda?p?p?.com\/invite|invite.gg|dsc.gg|urlcord.cf)\/[^\s/]+?(?=\b)/.test(message.content)) {
            if (data.automod.actions === "DELETE") {
                message.deletable ? message.delete() : false;
                embed.setDescription(`**${message.author}, Please don't send invite links.**\n> **Action:**\n\`-\` Message Delete`);
